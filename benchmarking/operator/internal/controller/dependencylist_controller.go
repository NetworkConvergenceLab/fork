...
func (r *DependencyListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	log.Info("Starting reconcile for dependency list")
	...
	log.Info("Fetched DependencyList", "spec", dependencyList.Spec)

	clusterName := dependencyList.Spec.ClusterName
	log.Info("Processing cluster", "clusterName", clusterName)

	clusterMap := dependencyList.Spec.ClusterServiceMap
	serviceList := clusterMap[clusterName]
	log.Info("Available services for cluster", "clusterName", clusterName, "services", serviceList)

	dependencies := dependencyList.Spec.Dependencies
	log.Info("Dependencies to check", "dependencies", dependencies)
	...
	if len(servicesForExport) == 0 {
		log.Info("No services found for export", "clusterName", clusterName)
	} else {
		log.Info("Services to export", "servicesForExport", servicesForExport)
	}

	err := exportServices(servicesForExport, req.NamespacedName.Namespace, log)
	if err != nil {
		log.Error(err, "unable to export")
		return ctrl.Result{}, err
	}

	log.Info("Successfully reconciled DependencyList", "namespace", req.Namespace, "name", req.Name)
	return ctrl.Result{}, nil
}

func exportServices(services []string, namespace string, log logr.Logger) error {
	...
	if err != nil {
		log.Error(err, "Failed to create Subctl producer")
		return err
	}
	...
	for _, service := range services {
		...
		if err != nil {
			log.Error(err, "Service export failed", "service", service, "namespace", namespace)
			return err
		}
		log.Info("Service export successful", "service", service, "namespace", namespace)
	}
	return nil
}
